...
def printTree(dossiers, niveau=0):
    indentation = '\t' * niveau
    print(indentation, dossiers[0])
    nds = 0
    nfs = 0
    nd = 0
    nf = 0
    for fichier in dossiers[1:]:
        if type(fichier) == list:
            nd, nf = printTree(fichier, niveau+1)
            nd += 1
        else:
            print('|' + indentation + fichier)
            nf += 1
    return nd + nds, nf + nfs
x,y = printTree(courant)
